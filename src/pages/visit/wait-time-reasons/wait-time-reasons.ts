...
export class WaitTimeReasonsPage {
  ...
  ngOnInit() {
    this.selectedReasons = this.waitActivity.waitReason.length > 0 ? this.waitActivity.waitReason : [];
  ...
  onSave(selectedReasons: string[]) {
    ...
    if (selectedReasons.indexOf(WAIT_TIME_REASONS.SITE_ISSUE) > -1 && this.notes === '' || selectedReasons.indexOf(WAIT_TIME_REASONS.OTHER) > -1 && this.notes === '') {
      CONFIRM.present();
    } else {
  ...
  onCheck(reason: AbandonmentReasonItemModel, selectedReasons) {
    reason.isChecked ? selectedReasons.push(reason.text) : selectedReasons.splice(selectedReasons.indexOf(reason.text), 1);
  }
