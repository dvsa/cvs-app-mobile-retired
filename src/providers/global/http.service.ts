...
export class HTTPService {
  ...
  getOpenVisitCheck(testerStaffId: string): Observable<HttpResponse<boolean>> {
    return this.http.get<boolean>(`${AppConfig.BACKEND_URL_VISIT}/open?testerStaffId=${testerStaffId}`, {
      observe: 'response'
    }).pipe(
  ...
  getApplicationVersion(): Promise<HttpResponse<LatestVersionModel>> {
    return this.http
      .get<LatestVersionModel>(AppConfig.URL_LATEST_VERSION, { observe: 'response' })
      .toPromise();
  }
}
