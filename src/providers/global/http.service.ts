...
export class HTTPService {
  ...
  getAtfs(): Observable<HttpResponse<TestStationReferenceDataModel[]>> {
    return this.http.get<TestStationReferenceDataModel[]>(AppConfig.app.BACKEND_URL_TEST_STATIONS, {
      observe: 'response'
    });
  }
  ...
  getTechRecords(
    ...
    return this.http.get<VehicleTechRecordModel[]>(
      `${AppConfig.app.BACKEND_URL_TECHRECORDS}/${encodeURIComponent(param)}/${PATHS.TECH_RECORDS}`,
      {
        observe: 'response',
        params: {
  ...
  getOpenVisitCheck(testerStaffId: string): Observable<HttpResponse<boolean>> {
    return this.http.get<boolean>(`${AppConfig.app.BACKEND_URL_VISIT}/open?testerStaffId=${testerStaffId}`, {
      observe: 'response'
    }).pipe(
  ...
  saveSignature(staffId: string, signatureString: string): Observable<HttpResponse<any>> {
    return this.http.put(`${AppConfig.app.BACKEND_URL_SIGNATURE}${staffId}.base64`, signatureString, {
      observe: 'response'
    });
  }
